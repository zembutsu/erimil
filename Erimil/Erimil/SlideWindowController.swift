...
class SlideWindowController {
    ...
    // MARK: - #55: Spread Navigation Helpers
    ...
    private func shouldShowSinglePage(at index: Int) -> Bool {
        ...
        if CacheManager.shared.hasSinglePageMarker(for: source.url, at: index) { return true }
        
        // Last page
        if index >= storedEntries.count - 1 { return true }
        
        // Next page has marker (current page shown single, next will be too)
        if CacheManager.shared.hasSinglePageMarker(for: source.url, at: index + 1) { return true }
    ...
    private func goToNext() {
        ...
        if nextIndex < storedEntries.count {
            ...
            currentIndex = storedEntries.count - 1  // Go to last page
        } else if AppSettings.shared.loopWithinSource {
